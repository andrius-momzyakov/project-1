...
class ModelWithAuditFields(models.Model):
    created_by = models.ForeignKey(User, related_name=u'creator', verbose_name=u'Кем создано', blank=True, null=True)
    updated_by = models.ForeignKey(User, related_name=u'updater', verbose_name=u'Кем изменено', blank=True, null=True)
    created_at = models.DateTimeField(verbose_name=u'Когда создано', null=True, blank=True, default=datetime.now)
    updated_at = models.DateTimeField(verbose_name=u'Когда изменено', null=True, blank=True)
...
class MenuItem0(models.Model):
    ...
    SUBMENU_APPEARANCE = (('submenu', u'Субменю'), ('dropdown', u'Выпадающее'))

    label = models.CharField(verbose_name=u'Метка', max_length=80, null=True, blank=True)
    html_label = models.TextField(verbose_name=u'Метка (html)', max_length=200, null=True, blank=True)
    section = models.ForeignKey(Section, verbose_name=u'Раздел', null=True, blank=True)
    appearance = models.CharField(verbose_name=u'Представление для меню 2го ур.', max_length=50,
    ...
    is_prototype = models.BooleanField(verbose_name=u'Показывать как прототип', blank=True, default=True)
    ...
    def __unicode__(self):
        return self.label + (lambda: u' - ОТКЛ' if not self.is_active else '')()
...
class MenuItem1(models.Model):
    ...
    section = models.ForeignKey(Section, verbose_name=u'Раздел', null=True, blank=True)
    menu = models.ForeignKey(MenuItem0, verbose_name=u'Меню верхнего уровня', null=True)
    divider = models.BooleanField(verbose_name=u"Поставить разделитель после", default=False)
    order = models.IntegerField(verbose_name=u'Ключ для сортировки', null=True, blank=True)
    is_active = models.BooleanField(verbose_name=u'Активно', blank=True, default=True)
    is_prototype = models.BooleanField(verbose_name=u'Показывать как прототип', blank=True, default=False)
    groups = models.CharField(verbose_name='Группы (через ",")', max_length=1000, blank=True, null=True)
    authenticated_access = models.BooleanField(verbose_name='Для залогиненных only', blank=True, default=False)

    def __unicode__(self):
        if self.is_active:
            return u'{} @ {}'.format(self.label, self.menu.label)
        else:
            return u'{} @ {} - ОТКЛ'.format(self.label, self.menu.label)
...
class NewsEntry(ModelWithAuditFields):
    ...
    region = models.ForeignKey(Region, verbose_name=u'Регион', null=True, blank=True)
    day = models.DateTimeField(verbose_name=u'Дата публикации новости',
                               default=datetime.now)
    title = models.CharField(verbose_name=u'Заголовок', max_length=500)
    preview_txt = RichTextUploadingField(verbose_name=u'Анонс', null=True, blank=True, config_name='basic')
    preview_img = models.ImageField(verbose_name=u'Картинка к анонсу', upload_to='news_preview/%Y/%m/%d/', null=True,
                                    blank=True)
    preview_img_width_pct = models.IntegerField(verbose_name=u'Ширина превьюшки в % от ширины контейнера', default=50)
    publish = models.BooleanField(verbose_name=u'Опубликовать', blank=True, default=False)
    archive = models.BooleanField(verbose_name=u'В архиве', blank=True, default=False)
    is_top = models.BooleanField(verbose_name=u'Новость дня', blank=True, default=False)
    is_regulation_change = models.BooleanField(verbose_name=u'Изменение в нормативных документах', blank=True,
                                               default=False)
    comments_allowed = models.BooleanField(verbose_name='Разрешить каменты', default=True)
    body = RichTextUploadingField(verbose_name=u'Текст', blank=True, null=True)

    created_by = models.ForeignKey(User, related_name=u'site_report_creator', verbose_name=u'Кем создано', blank=True,
                                   null=True)
    updated_by = models.ForeignKey(User, related_name=u'site_report_updater', verbose_name=u'Кем изменено', blank=True,
                                   null=True)
    ...
    def __unicode__(self):
        return u'{} {:%d.%m.%Y %H_%M_%S} {}'.format(self.title, self.day, (lambda: self.created_by if self.created_by else '')())
